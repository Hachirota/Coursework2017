import readurl #importing the external readurl
import operator #importing the operator module used in sorting the list of averages

headers, body = readurl.get_url('http://mf2.dit.ie/googleprices.csv') #importing the data from an external site
...
for item in entries:
    ...
    if current_year != item[0][:4]: #check to see if the year being totalled has ended
        if yearly_price != 0:#prevent divide by zero errors
            yearly_average = yearly_price / yearly_volume #calucate the average for the year
...
# the final current year and month variables will match the final entry in the list.
...
#print the worst six and best six months
print(list_of_monthly_averages[:6])
print(list_of_monthly_averages[-6:])

#print the worst six and best six months
print(list_of_yearly_averages[:6])
print(list_of_yearly_averages[-6:])
